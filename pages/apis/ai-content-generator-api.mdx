...
`POST /v1/content/summarize` condenses long text into a short summary and key
points.
...
`POST /v1/content/rewrite` rephrases text in a target tone while preserving
meaning.
...
`POST /v1/content/meta-tags` generates an SEO title, meta description, and
...
`POST /v1/content/blog/outline` generates a structured blog outline from a topic.
...
`POST /v1/content/product-description` generates ecommerce product copy.
...
`POST /v1/content/social-caption` generates platform-specific captions with
optional hashtags.
...
`POST /v1/content/headlines` generates headline variants with an angle and a
...
`POST /v1/content/faq` generates question-and-answer pairs from source text.
