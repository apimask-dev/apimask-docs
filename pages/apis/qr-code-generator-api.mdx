...
`POST /v1/qr/generate` encodes any text or URL. Plus all [rendering options](#rendering-options).
...
`POST /v1/qr/url` encodes a URL with validation. Plus all [rendering options](#rendering-options).
...
`POST /v1/qr/wifi` encodes WiFi credentials so a scan connects automatically.
...
`POST /v1/qr/vcard` encodes contact details as a scannable vCard. Plus all
[rendering options](#rendering-options).
...
`POST /v1/qr/email` encodes a pre-filled email link. Plus all
[rendering options](#rendering-options).
...
`POST /v1/qr/sms` encodes a pre-filled SMS. Plus all
[rendering options](#rendering-options).
